

import { motion } from 'framer-motion'

import { pricingPages, pricingCategories } from '@/config/pricing'

export default function PricingPageClient() {
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900">
      {/* Hero Section */}
      <section className="relative py-20 overflow-hidden">
        <div className="absolute inset-0">
          <div className="absolute top-20 right-20 w-72 h-72 bg-green-500/20 rounded-full blur-3xl"></div>
          <div className="absolute bottom-20 left-20 w-72 h-72 bg-emerald-500/20 rounded-full blur-3xl"></div>
        </div>

        <div className="container mx-auto px-4 relative z-10">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="text-center max-w-4xl mx-auto"
          >
            <span className="inline-block px-4 py-2 bg-green-500/20 text-green-400 rounded-full text-sm font-medium mb-6">
              💰 دليل الأسعار
            </span>
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">
              كم <span className="text-green-500">تكلفة</span> الخدمات؟
            </h1>
            <p className="text-xl text-gray-300 leading-relaxed mb-8">
              أسعار شفافة ودليل شامل لتكاليف الخدمات الرقمية
              <br />
              اعرف التكلفة الحقيقية قبل ما تبدأ
            </p>
          </motion.div>
        </div>
      </section>

      {/* Categories */}
      <section className="py-8 sticky top-0 bg-gray-900/80 backdrop-blur-lg z-20 border-b border-gray-700">
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-3">
            {pricingCategories.map((cat) => (
              <a
                key={cat.id}
                href={`#${cat.id}`}
                className="px-4 py-2 bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white rounded-full transition-all text-sm flex items-center gap-2"
              >
                <span>{cat.icon}</span>
                <span>{cat.nameAr}</span>
              </a>
            ))}
          </div>
        </div>
      </section>

      {/* Pricing Pages by Category */}
      {pricingCategories.map((category) => {
        const categoryPages = pricingPages.filter(p => p.category === category.id)

        return (
          <section key={category.id} id={category.id} className="py-16">
            <div className="container mx-auto px-4">
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                className="mb-10"
              >
                <div className="flex items-center gap-4 mb-4">
                  <span className="text-4xl">{category.icon}</span>
                  <h2 className="text-3xl font-bold text-white">أسعار {category.nameAr}</h2>
                </div>
                <div className="w-20 h-1 bg-gradient-to-r from-green-500 to-emerald-500 rounded-full"></div>
              </motion.div>

              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {categoryPages.map((pricing, index) => (
                  <motion.div
                    key={pricing.id}
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ delay: index * 0.1 }}
                  >
                    <a href={`/pricing/${pricing.slug}`}>
                      <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-6 border border-gray-700 hover:border-green-500/50 transition-all duration-300 hover:-translate-y-2 group h-full">
                        {/* Title */}
                        <h3 className="text-xl font-bold text-white mb-4 group-hover:text-green-400 transition-colors">
                          {pricing.titleAr}
                        </h3>

                        {/* Price Range Preview */}
                        <div className="space-y-3 mb-6">
                          {pricing.pricingTiers.slice(0, 3).map((tier, idx) => (
                            <div key={idx} className={`flex justify-between items-center text-sm ${tier.popular ? 'bg-green-500/10 rounded-lg p-2' : ''}`}>
                              <span className="text-gray-400">{tier.name}</span>
                              <span className={`font-bold ${tier.popular ? 'text-green-400' : 'text-white'}`}>
                                {tier.priceRange.split(' ')[0]} ريال
                              </span>
                            </div>
                          ))}
                        </div>

                        {/* Keywords */}
                        <div className="flex flex-wrap gap-2 mb-4">
                          {pricing.keywordsAr.slice(0, 2).map((keyword, idx) => (
                            <span key={idx} className="px-2 py-1 bg-gray-700/50 text-gray-400 rounded-full text-xs">
                              {keyword}
                            </span>
                          ))}
                        </div>

                        {/* CTA */}
                        <div className="text-center">
                          <span className="text-green-400 group-hover:text-green-300 text-sm font-medium">
                            شوف التفاصيل الكاملة ←
                          </span>
                        </div>
                      </div>
                    </a>
                  </motion.div>
                ))}
              </div>
            </div>
          </section>
        )
      })}

      {/* CTA */}
      <section className="py-20 bg-gradient-to-r from-green-900/30 to-emerald-900/30">
        <div className="container mx-auto px-4">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="text-center max-w-3xl mx-auto"
          >
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">
              محتاج عرض سعر مخصص؟
            </h2>
            <p className="text-xl text-gray-300 mb-8">
              احكيلنا عن مشروعك وراح نرسل لك عرض سعر خلال 24 ساعة
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a
                href="https://wa.me/966XXXXXXXXX"
                className="px-8 py-4 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl transition-all flex items-center justify-center gap-3"
              >
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                اطلب عرض سعر
              </a>
              <a
                href="/contact"
                className="px-8 py-4 bg-gray-700 hover:bg-gray-600 text-white font-bold rounded-xl transition-all"
              >
                تواصل معنا
              </a>
            </div>
          </motion.div>
        </div>
      </section>
    </main>
  )
}
